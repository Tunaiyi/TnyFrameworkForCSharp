...
public abstract class ControllerHolder
{
        ...
        AuthenticationRequiredAttribute? authAttribute, AppProfileAttribute? appProfile, ScopeProfileAttribute? scopeProfile)
    {
        ...
        ContactGroups = CopyOnExist(authAttribute?.ContactGroups);
        AppTypes = CopyOnExist(appProfile?.AppTypes);
        Scopes = CopyOnExist(scopeProfile?.Scopes);
    }

    private static IList<string>? CopyOnExist(string[]? value)
    {
