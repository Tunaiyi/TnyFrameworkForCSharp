...
{
    private static readonly ILogger LOGGER = LogFactory.Logger<LifecycleProcessor>();

    private static readonly Dictionary<LifecycleStage, SortedSet<Lifecycle>> LIFECYCLE_MAP = new();

    private static readonly Dictionary<Lifecycle, List<ILifecycleHandler>> HANDLER_MAP = new();
    ...
    public Task OnPrepareStart(bool errorContinue = false)
    {
        return Process<IAppPrepareStart, PrepareStarter>(
            LifecycleStage.PREPARE_START, nameof(IAppPrepareStart.OnPrepareStart), handler => handler.OnPrepareStart(), errorContinue);
    }

    public Task OnPostStart(bool errorContinue = false)
    {
        return Process<IAppPostStart, PostStarter>(
            LifecycleStage.POST_START, nameof(IAppPostStart.OnPostStart), handler => handler.OnPostStart(), errorContinue);
    }

    public Task OnClosed(bool errorContinue = false)
    {
        return Process<IAppClosed, PostCloser>(
