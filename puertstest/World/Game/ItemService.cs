...
using TnyFramework.Coroutines.Helios.Concurrency;
namespace puertstest.World.Game
{
    public class ItemService : ItemModelProvider
    {
        ...
        private volatile PuertsManager puertsManager;
        ...
        private void InitPuerts()
        {
            if (puertsManager != null)
            {
                return;
            }
            lock (this)
            {
                ...
                THREAD_POOL.QueueUserWorkItem(() => {
                    try
                    {
                        if (puertsManager != null)
                            return;
                        puertsManager = PuertsManager.Create();
        ...
        public void Init(bool loadAll = false)
        {
            InitPuerts();
        ...
        private T Eval<T>(string script)
        {
            ...
            THREAD_POOL.QueueUserWorkItem(() => {
                try
                {
                    ...
                    var model = puertsManager.Env.Eval<T>(script);
